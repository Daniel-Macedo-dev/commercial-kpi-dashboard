...
| Bilingual UI | Full English / Português toggle — all labels, filter controls (expander+checkboxes with translated option values), native calendar date range filter with translated labels and localized date format (DD/MM/YYYY in PT, YYYY/MM/DD in EN), charts, diagnostics, and insights. Fictional dataset values remain in English internally; translated values are display-only and do not affect filtering, calculations, or CSV exports. |
